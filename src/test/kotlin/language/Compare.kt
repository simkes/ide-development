...
fun compare(node1: ASTNode?, node2: ASTNode?): Boolean {
    ...
    return when (node1) {
        ...
        is Expr.SymbolName -> compareSymbolName(node1, node2 as Expr.SymbolName)
...
fun compareVarDeclaration(node1: Stmt.VarDeclaration, node2: Stmt.VarDeclaration): Boolean {
    return compareSymbolName(node1.symbol, node2.symbol) && compare(node1.expr, node2.expr)
}
...
fun compareFuncDeclaration(node1: Stmt.FuncDeclaration, node2: Stmt.FuncDeclaration): Boolean {
    return compareSymbolName(node1.symbol, node2.symbol) &&
...
fun compareProcCall(node1: Stmt.ProcCall, node2: Stmt.ProcCall): Boolean {
    return compareSymbolName(node1.symbol, node2.symbol) && compare(node1.arguments, node2.arguments)
}

fun compareParameter(node1: Parameter, node2: Parameter): Boolean {
    return compareSymbolName(node1.symbol, node2.symbol) && (node1.type == node2.type)
}
...
fun compareSymbolName(node1: Expr.SymbolName, node2: Expr.SymbolName): Boolean {
...
fun compare(error1: Error, error2: Error): Boolean {
    if (error1.type != error2.type) return false
    return when (error1) {
        is LexicalError -> compare(error1.token.token, (error2 as LexicalError).token.token)
        is SyntaxError -> error1.tokenIndex == (error2 as SyntaxError).tokenIndex
        is SemanticError -> compare(error1.node, (error2 as SemanticError).node)
    }
}